...
function Transcoder(source) {
	...
	Transcoder.prototype.metadata = function(key, value) {
	  var args = [ '-' + key ];
	  if (value !== undefined) {
	    args.push(value);
	  }
	  if(this.args["metadata"] === undefined) {
	    this.args["metadata"] = [];
	  }
	  this.args["metadata"].push([key, value]);
	  return this;
	};
	...
	Transcoder.prototype._compileArguments = function () {
	  ...
	  for (var key in this.args) {
	    if (this.args[key].length !== 5) {
	      a = a.concat(this.args[key]);
	    }
	  }
	  for (var key in this.args) {
	    if (this.args[key].length === 5) {
	      console.log("--------------");
	      _.each(this.args[key], function(metadata) {
	        console.log("-metadata " + metadata[0] + "=" + metadata[1]);
	        a.push("-metadata");
	        a.push(metadata[0] + "=" + metadata[1]);
	      });
	    }
	  }
	    console.log("1111111");
